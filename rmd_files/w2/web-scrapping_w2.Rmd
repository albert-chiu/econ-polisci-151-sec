...
We can represent html documents using a document object model (DOM). To do so, we would use a data structure called a _tree_. Trees are composed of _nodes_ and have a hierarchical structure: there is one "root" node, and all other nodes branch out from it. Consider the following example:
```{r}
eg_html <- "<html>
